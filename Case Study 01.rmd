...
last_df <- rbind(ale, ipa, lager, stout, other)
last_df$Style_Group <- as.factor(last_df$Style_Group)

# Table with mean ABVs / IBUs per Group
last_df %>% group_by(Style_Group) %>% summarize(meanABV = mean(ABV), meanIBU = mean(IBU))
