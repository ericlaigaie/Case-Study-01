...
---
# Introduction: In this presentation, we will be visualizing and interpreting the characteristics of beers and the breweries that make those beers. Key variables that will be considered for inference include beer alcohol content, beer international bitterness units and brewery locations. The goal of this presentation is to provide feedback and responses to the questions you have proposed. We plan to do this by sharing key insights gathered through our analysis but without the statistical jargon. At the end of the presentation, we plan to share additional insight that was not requested but is imperative to Budweiser if the company plans to produce new beers in the near-term future.
...
new_df$Style_Group = factor(new_df$Style_Group)
...
trainInd = sample(seq(1,1534,1), .7*1534)
train = new_df[trainInd,]
test = new_df[-trainInd,]

# Initial run of KNN model with K=5 
classifications = knn(train[,c(12,13)],test[,c(12,13)],train$Style_Group,prob = TRUE, k = 5)
...
for(j in 1:iterations)
{
  ...
  for(i in 1:numks)
  {
    classifications = knn(train[,c(12,13)],test[,c(12,13)],train$Style_Group, prob = TRUE, k = i)
...
# K = 19 gives highest accuracy (92%)
# K = 19 gives highest sensitivity (94%)
# K = 27 gives highest specificity (89%)

# Run KNN model with K=6 for highest accuracy (per hyper tuning)
classifications = knn(train[,c(12,13)],test[,c(12,13)],train$Style_Group,prob = TRUE, k = 19)
confusionMatrix(table(classifications,test$Style_Group))
...
model = naiveBayes(train_nb[,c(12,13)],train_nb$Style_Group,laplace = 1)
table(predict(model,test_nb[,c(12,13)]),test_nb$Style_Group)
CM = confusionMatrix(table(predict(model,test_nb[,c(12,13)]),test_nb$Style_Group))
CM
...
# adding in state regions to find mean ABV/IBU per region
regions = as.data.frame(read.csv('/Users/alexanderlopez/Documents/SMU/Classes/MSDS_6306_Doing_Data_Science/Case_Study_1/Case-Study-01/regions_v2.csv', header = TRUE))
...
new_df2$Region = as.factor(new_df2$Region)

new_df2 %>% group_by(Region) %>% summarise(meanABV = mean(ABV)) %>% ggplot(aes(x=meanABV, y=Region)) + geom_bar(stat = 'identity')

new_df2 %>% group_by(Region) %>% summarise(meanIBU = mean(IBU)) %>% ggplot(aes(x=meanIBU, y=Region)) + geom_bar(stat = 'identity')

new_df2 %>% group_by(Region) %>% ggplot(aes(x=ABV,fill=Style_Group)) + geom_boxplot() + labs(title = 'ABV by Region by Style Group') + facet_wrap(~Region)

new_df2 %>% group_by(Region) %>% ggplot(aes(x=ABV,fill=Region)) + geom_boxplot() + labs(title = 'ABV by Style Group by Region') + facet_wrap(~Style_Group)

library(GGally)
new_df2 %>% 
select(Region, ABV, IBU, Style_Group) %>%
ggpairs(aes(color = Region))

new_df2 %>% 
select(Region, ABV, IBU, Style_Group) %>%
ggpairs(aes(color = Style_Group))

# scatterplot facet wrap (region), ibu vs abv per style

new_df2 %>% ggplot(aes(x=ABV,y=IBU,col=Style_Group)) + geom_point() + labs(title = 'ABV vs IBU by Region by Style Group') + geom_smooth(method = 'lm')+ facet_wrap(~Region)
...
# dropping dummy variables to clean df
# new_df2 = subset(new_df2, select = -c(Ale,IPA,Lager,Lager,Stout,Other))

```

# Conclusion: In conclusion, we found that the breweries in the US are mostly located in high density states like Texas and California. We saw that the median beer ABV does not vary all that much between states but also that there are a large number of beers with high ABV’s. This observation is consistent with the idea that craft beers are becoming more and more popular in the US. We also saw a moderate correlation between ABV and IBU, probably because adding hops, a key ingredient in most beers, will make a beer more alcoholic and more bitter. Although the craft beer trend is rising, Budweiser would likely benefit from a lower ABV beer, possibly because most people do not enjoy the bitterness. To further analyze the correlation between ABV and IBU, particularly in Ales vs IPA’s, we ran the data through a KNN model. The model came to show that, given a beer’s ABV and IBU, we could predict whether that beer was an Ale or IPA with an accuracy of over 91%! We also had very similar accuracy results when using a Naïve-Bayes model. In both cases, the models showed that ABV and IBU do not significantly vary in beer styles. Finally, our statistical test (Rank Sum) showed the significance in differing mean ABV between regions. The test indicated that when Budweiser is ready to implement a new beer, the ABV mean in the region of interest, as well as the surrounding regions, should all be carefully considered.